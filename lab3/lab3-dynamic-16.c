...
//    #pragma omp parallel for default(none) private(i) shared(m1, max, A, cycle) ordered schedule(dynamic)
    #pragma omp parallel for default(none) private(i) shared(m1, max, A, cycle) schedule(dynamic, 16)
    for(i=0;i<max;i++)
    {
...
void generate_m2(double *m2, int max, int A, unsigned int *cycle)
{
    ...
    #pragma omp parallel for default(none) private(i) shared(m2, max, A, cycle) schedule(dynamic, 16)
    for(i=0;i<max;i++)
    {
        int random = rand_r(cycle);
...
{
    int i;
    #pragma omp parallel for default(none) private(i) shared(arr, size) schedule(dynamic, 16)
    for(i=0;i<size;i++)
    {
...
//        printf("\nM2: %d, BEFORE ADD operation: %f", i, arr[i]);
    ...
    #pragma omp parallel for default(none) private(i) shared(input, res, size) schedule(dynamic, 16)
    for(i=size;i>0;i--)
...
{
    int i;
    #pragma omp parallel for default(none) private(i) shared(m1, m2, size) schedule(dynamic, 16)
    for(i=0;i<size;i++)
    {
...
double reduce(double *m2, int size)
{
    ...
    int i;
    #pragma omp parallel for default(none) private(i) shared(m2, size, min) reduction(+:sum) schedule(dynamic, 16)
    for(i=0;i<size;i++)
    {
...
int main(int argc, char *argv[])
{
    ...
    #pragma omp parallel for default(none) private(i) shared(A, N, total, results) schedule(dynamic, 16)
	for(i=1;i<=total;i++)
	{
